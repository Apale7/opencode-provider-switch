...
func targetActionsForAlias(alias config.Alias) []string {
	if alias.AutoGenerated {
		return []string{string(diagnostics.ActionRebindTarget), string(diagnostics.ActionDeleteAlias), string(diagnostics.ActionKeep)}
	}
	if alias.Locked {
		return []string{string(diagnostics.ActionDeleteAlias), string(diagnostics.ActionKeep)}
	}
