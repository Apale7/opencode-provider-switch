...
// PlanAliasUpgrade converts auto/locked alias to canonical manual ownership.
func PlanAliasUpgrade(base *config.Config, baseRevision, aliasName string) (Result, error) {
	...
	if !alias.AutoGenerated {
		// Idempotent no-op for already-manual.
		plan.NoOp = true
		finalizePlan(&plan)
	...
	plan.RequestedChanges = append(plan.RequestedChanges, Change{
		ID:         "requested:upgrade_alias:" + aliasName,
		Kind:       ChangeUpdate,
		Source:     SourceRequested,
		Entity:     EntityAlias,
		ReasonCode: ReasonUpgradeOwnership,
		Path:       fmt.Sprintf("/config/aliases/%d", idx),
		Params:     params("alias", aliasName, "wasLocked", alias.Locked),
	})
	finalizePlan(&plan)
	if !plan.Executable {
		return Result{Plan: plan}, nil
	...
	a := &candidate.Aliases[idx]
	a.AutoGenerated = false
	a.Locked = false
	for ti := range a.Targets {
		a.Targets[ti].AutoGenerated = false
	}
	return Result{Plan: plan, Candidate: candidate}, nil
}

// PlanAliasMutateGate rejects auto/locked mutations before any candidate change.
func PlanAliasMutateGate(base *config.Config, baseRevision, aliasName, intent string) (Result, error) {
	...
	if err := RequireManualAliasMutation(base, aliasName); err != nil {
		code := ReasonUpgradeRequired
