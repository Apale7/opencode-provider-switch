...
func (m model) aliasButtonClick(key string) (tea.Model, tea.Cmd) {
	...
	case "unbind":
		...
		if a.AutoGenerated && t.AutoGenerated {
			m.err = m.t("error.systemTargetDisableOnly")
			return m, nil
		}
	...
	case "reset-order":
		...
		return m, m.resetAliasTargetOrder(*a)
	case "remove":
		a := m.selectedAlias()
		if a == nil {
			m.err = m.t("error.noAlias")
...
func (m model) viewAliases() string {
	...
	t := table.New().Border(lipgloss.NormalBorder()).BorderStyle(ruleStyle).Wrap(false).Headers("", "Alias", "Kind", "State", "Protocol", "Order", "Targets", "Available")
	for i, a := range m.aliases {
		state := m.t("alias.state.enabled")
		if !a.Enabled {
			state = m.t("alias.state.disabled")
		}
		kind := m.t("alias.kind.custom")
		order := "-"
		if a.AutoGenerated {
			kind = m.t("alias.kind.autoModel")
			order = m.t("alias.order.provider")
			if a.TargetOrderMode == "custom" {
				order = m.t("alias.order.custom")
			}
		}
		marker := ""
		if i == m.aliasIndex {
		...
		t.Row(marker, a.Alias, kind, state, a.Protocol, order, strconv.Itoa(a.TargetCount), strconv.Itoa(a.AvailableTargetCount))
	}
	t.StyleFunc(func(row int, col int) lipgloss.Style {
		if row == table.HeaderRow {
			return sectionStyle
...
func (m model) aliasButtons() []tuiButton {
	...
	return []tuiButton{
		...
		{key: "reset-order", label: m.t("action.resetOrder")},
		{key: "remove", label: m.t("action.remove")},
		{key: "refresh", label: m.t("action.refresh")},
	}
}
...
func (m model) resetAliasTargetOrder(alias app.AliasView) tea.Cmd {
	return func() tea.Msg {
		_, err := m.svc.ResetAliasTargetOrder(m.ctx, app.AliasLockInput{Name: alias.Alias})
		if err != nil {
			return refreshedMsg{err: err}
		}
		return runRefresh(m, m.t("status.targetOrderReset", alias.Alias))
	}
}
