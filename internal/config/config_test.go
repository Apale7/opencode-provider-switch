...
func TestAutoGenerateAliases_CreatesNewAlias(t *testing.T) {
	...
	if got := cfg.FindAlias("gpt-test"); got == nil || !got.AutoGenerated {
		t.Fatalf("FindAlias should return existing auto alias, got %#v", got)
	}
}
...
func TestAutoGenerateAliases_MigratesCatalogManualAlias(t *testing.T) {
	t.Parallel()
	...
	created, updated := cfg.AutoGenerateAliases(cfg.Providers[0])
	if len(created) != 0 || !slices.Equal(updated, []string{"manual-model"}) {
		t.Fatalf("created=%#v updated=%#v, want updated manual-model", created, updated)
	}
	got := cfg.FindAlias("manual-model")
	if got == nil {
		t.Fatal("catalog alias disappeared")
	}
	if !got.AutoGenerated {
		t.Fatal("catalog-matching manual alias was not absorbed as auto-generated")
	}
	if got.TargetOrderMode != TargetOrderModeCustom {
		t.Fatalf("target order mode = %q, want custom", got.TargetOrderMode)
	}
	wantTargets := []Target{
		original.Targets[0],
		{Provider: "p1", Group: DefaultGroupID, Model: "manual-model", Enabled: true, AutoGenerated: true},
	}
...
func TestFindAliasReturnsExistingAlias(t *testing.T) {
	t.Parallel()
	...
	if got := cfg.FindAlias("auto-only"); got == nil || !got.AutoGenerated {
		t.Fatalf("FindAlias should find auto-only, got %#v", got)
	}
	...
	if cfg.FindManualAlias("manual-only") == nil {
		t.Fatal("FindManualAlias should find manual-only")
	}
	if cfg.FindManualAlias("auto-only") != nil {
		t.Fatal("FindManualAlias must ignore auto-generated aliases")
	}
...
func TestSetProviderPriority_PreservesCustomAutoTargetOrder(t *testing.T) {
	t.Parallel()
	...
	cfg := &Config{
		...
		Aliases: []Alias{{
			...
			Enabled:         true,
			AutoGenerated:   true,
			TargetOrderMode: TargetOrderModeCustom,
			Targets: []Target{
				{Provider: "p1", Group: DefaultGroupID, Model: "custom-order", Enabled: true, AutoGenerated: true},
				{Provider: "p2", Group: DefaultGroupID, Model: "custom-order", Enabled: true, AutoGenerated: true},
			},
		}},
	}

	cfg.SetProviderPriority([]string{"p2", "p1"})
	got := cfg.FindAlias("custom-order")
	if got == nil || got.Targets[0].Provider != "p1" || got.Targets[1].Provider != "p2" {
		t.Fatalf("custom order changed: %#v", got)
	}

	if err := cfg.ResetAliasTargetOrder("custom-order"); err != nil {
		t.Fatalf("ResetAliasTargetOrder() error = %v", err)
	}
	got = cfg.FindAlias("custom-order")
	if got.TargetOrderMode != "" || got.Targets[0].Provider != "p2" || got.Targets[1].Provider != "p1" {
		t.Fatalf("reset order = %#v, want provider priority p2,p1", got)
	}
}
