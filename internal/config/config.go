...
const (
	// TargetOrderModeProviderPriority means auto model alias targets follow the
	// configured Provider Priority. Empty values are treated the same for legacy configs.
	TargetOrderModeProviderPriority = "provider_priority"
	// TargetOrderModeCustom means the user has explicitly reordered targets; newly
	// discovered system targets are appended instead of reordering existing targets.
	TargetOrderModeCustom = "custom"
)
...
type Alias struct {
	...
	TargetOrderMode string   `json:"target_order_mode,omitempty"` // 空/provider_priority=Provider 优先级；custom=用户自定义顺序
}
...
func (c *Config) marshalPersistentLocked() ([]byte, error) {
	...
	type persistAlias struct {
		...
		TargetOrderMode string          `json:"target_order_mode,omitempty"`
	}
	...
	for _, a := range aliases {
		...
		outAliases = append(outAliases, persistAlias{
			...
			Locked:          a.Locked,
			TargetOrderMode: storageTargetOrderMode(a.TargetOrderMode),
		})
	}
...
func finalizeLoadedConfig(path string, data []byte) (*Config, error) {
	...
	c.NormalizeCatalogAliasOwnership()
...
// FindAlias returns the unique alias with matching name or nil, regardless of
// whether it is system-owned or user-defined.
...
// FindManualAlias returns the user-defined alias with matching name or nil.
func (c *Config) FindManualAlias(name string) *Alias {
	c.mu.RLock()
	defer c.mu.RUnlock()
	name = strings.TrimSpace(name)
	for i := range c.Aliases {
...
// AliasNameMatchesProviderModel reports whether name is present in any Provider
// Group model catalog for protocol. Empty protocol matches any catalog protocol.
func (c *Config) AliasNameMatchesProviderModel(name, protocol string) bool {
	c.mu.RLock()
	defer c.mu.RUnlock()
	_, ok := c.catalogAliasProtocolLocked(name, protocol)
	return ok
}

// NormalizeCatalogAliasOwnership absorbs legacy manual aliases whose names now
// match the Provider Group model catalog. User-visible metadata, enabled state,
// existing target order, target enabled state, and custom targets are preserved.
func (c *Config) NormalizeCatalogAliasOwnership() (converted []string) {
	c.mu.Lock()
	defer c.mu.Unlock()
	seen := map[string]bool{}
	for i := range c.Aliases {
		...
		protocol, ok := c.catalogAliasProtocolLocked(a.Alias, a.Protocol)
		if !ok {
			continue
		}
		...
		if len(a.Targets) > 0 && NormalizeAliasTargetOrderMode(a.TargetOrderMode) == TargetOrderModeProviderPriority {
			a.TargetOrderMode = TargetOrderModeCustom
		}
		if a.Locked && NormalizeAliasTargetOrderMode(a.TargetOrderMode) == TargetOrderModeProviderPriority {
			a.TargetOrderMode = TargetOrderModeCustom
		}
		if !seen[a.Alias] {
			seen[a.Alias] = true
			converted = append(converted, a.Alias)
		}
	}
	return converted
}
...
// Legacy manual aliases whose name matches the model catalog are absorbed as
// auto model aliases while preserving user overrides. Disabled providers,
// disabled groups, and auto-alias switches (global or per-provider) produce zero changes.
func (c *Config) AutoGenerateAliases(provider Provider) (created []string, updated []string) {
	...
	for _, group := range provider.Groups {
		...
		for _, model := range group.Models {
			...
			if !a.AutoGenerated {
				if strings.TrimSpace(a.Protocol) != "" && !ProtocolsMatch(a.Protocol, protocol) {
					continue
				}
				a.AutoGenerated = true
				if strings.TrimSpace(a.Protocol) == "" {
			...
			a.Targets = c.mergeNewAutoTargetsLocked(a.Targets, a.TargetOrderMode, []Target{target})
...
func (c *Config) catalogAliasProtocolLocked(name, protocol string) (string, bool) {
	name = strings.TrimSpace(name)
	...
	for _, provider := range c.Providers {
		for _, group := range provider.Groups {
			groupProtocol := NormalizeProviderProtocol(group.Protocol)
			if protocol != "" && !ProtocolsMatch(protocol, groupProtocol) {
				continue
			}
			if providerModelsContain(group.Models, name) {
				return NormalizeAliasProtocol(groupProtocol), true
			}
		}
	}
	return "", false
...
// NormalizeAliasTargetOrderMode returns the effective order mode for an alias.
// Legacy empty values are equivalent to provider_priority.
func NormalizeAliasTargetOrderMode(mode string) string {
	switch strings.TrimSpace(mode) {
	case TargetOrderModeCustom:
		return TargetOrderModeCustom
	default:
		return TargetOrderModeProviderPriority
	}
}

func storageTargetOrderMode(mode string) string {
	if NormalizeAliasTargetOrderMode(mode) == TargetOrderModeCustom {
		return TargetOrderModeCustom
	}
	return ""
}
...
func (c *Config) reorderUnlockedAutoAliasTargetsLocked() {
	for i := range c.Aliases {
		...
		if NormalizeAliasTargetOrderMode(a.TargetOrderMode) == TargetOrderModeCustom {
			continue
		}
...
func (c *Config) mergeNewAutoTargetsLocked(targets []Target, orderMode string, additions []Target) []Target {
	if len(additions) == 0 {
		return cloneTargets(targets)
	}
	batch := c.sortTargetsByPriorityLocked(additions)
	out := cloneTargets(targets)
	out = append(out, batch...)
	if NormalizeAliasTargetOrderMode(orderMode) == TargetOrderModeCustom {
		return out
	}
	return c.sortTargetsByPriorityLocked(out)
}
...
func (c *Config) AddTarget(alias string, t Target) error {
	...
	for i := range c.Aliases {
		if c.Aliases[i].Alias == alias {
			...
			if c.Aliases[i].AutoGenerated && !t.AutoGenerated {
				c.Aliases[i].TargetOrderMode = TargetOrderModeCustom
			}
			return nil
		}
	}
...
// ResetAliasTargetOrder restores an alias to Provider Priority target ordering.
func (c *Config) ResetAliasTargetOrder(alias string) error {
	c.mu.Lock()
	defer c.mu.Unlock()
	...
	for i := range c.Aliases {
		...
		c.Aliases[i].TargetOrderMode = ""
		c.Aliases[i].Targets = c.sortTargetsByPriorityLocked(c.Aliases[i].Targets)
		return nil
	}
	return fmt.Errorf("alias %q not found", alias)
}
...
func cloneAlias(a Alias) Alias {
	a.Protocol = NormalizeAliasProtocol(a.Protocol)
	a.TargetOrderMode = storageTargetOrderMode(a.TargetOrderMode)
...
func normalizeAliases(aliases []Alias) {
	for i := range aliases {
		...
		aliases[i].TargetOrderMode = storageTargetOrderMode(aliases[i].TargetOrderMode)
		if aliases[i].AutoGenerated && aliases[i].Locked && aliases[i].TargetOrderMode == "" {
			aliases[i].TargetOrderMode = TargetOrderModeCustom
		}
