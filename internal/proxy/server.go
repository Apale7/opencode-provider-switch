...
func (s *Server) handleProtocolRequest(protocol string, w http.ResponseWriter, r *http.Request) {
	...
	// Alias-only resolution: existing manual and auto model aliases share one path.
	...
	if alias != nil && alias.AutoGenerated {
		aliasSource = "auto"
		s.logger.Printf("req=%d alias=%q resolved via auto alias source=auto", reqID, aliasName)
	}
	if alias == nil {
