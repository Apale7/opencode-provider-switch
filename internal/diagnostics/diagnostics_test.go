...
func TestScanConfigClassifiesReferencesWithoutModelForeignKey(t *testing.T) {
	cfg := config.Default()
	cfg.Providers = []config.Provider{{ID: "disabled", Disabled: true, Groups: []config.ProviderGroup{{ID: "default", Protocol: config.ProtocolOpenAIResponses}}}, {ID: "anthropic", Groups: []config.ProviderGroup{{ID: "default", Protocol: config.ProtocolAnthropicMessages}}}}
	...
	cfg.RequestRewriteRules = []config.RequestRewriteRule{{Name: "wild", Alias: "unknown", ProviderGroups: []config.ProviderGroupSelector{}}, {Name: "single", Alias: "chat", ProviderGroups: []config.ProviderGroupSelector{{Provider: "gone", Group: "default"}}}}
	...
	for _, code := range []Code{CodeAliasTargetProviderMissing, CodeAliasTargetProviderDisabled, CodeAliasTargetProtocolMismatch, CodeRewriteAliasUnresolved, CodeRewriteProviderGroupMissing, CodePriorityProviderMissing, CodeAliasNoAvailableTarget} {
		if _, ok := codes[code]; !ok {
			t.Errorf("missing code %s", code)
		}
	}
	if actions := codes[CodeRewriteProviderGroupMissing].AllowedActions; contains(actions, ActionRemoveSelector) {
...
func TestScanConfigWildcardAndOwnershipActions(t *testing.T) {
	cfg := config.Default()
	cfg.Providers = []config.Provider{{ID: "p", Groups: []config.ProviderGroup{{ID: config.DefaultGroupID, Protocol: config.ProtocolOpenAIResponses}}}}
	...
	cfg.RequestRewriteRules = []config.RequestRewriteRule{{Name: "all", Alias: "auto", ProviderGroups: []config.ProviderGroupSelector{}}}
...
func TestProviderGroupDiagnosticsAreExactAndIsolated(t *testing.T) {
	cfg := config.Default()
	cfg.Providers = []config.Provider{
		{ID: "p", Groups: []config.ProviderGroup{
			{ID: "alpha", Protocol: config.ProtocolOpenAIResponses, APIKeys: []string{"upstream-secret-alpha"}},
			{ID: "beta", Protocol: config.ProtocolAnthropicMessages, APIKeys: []string{"upstream-secret-beta"}},
			{ID: "disabled", Protocol: config.ProtocolOpenAIResponses, Disabled: true},
		}},
	}
	cfg.Aliases = []config.Alias{{Alias: "chat", Protocol: config.ProtocolOpenAIResponses, Enabled: true, Targets: []config.Target{
		{Provider: "p", Group: "missing", Model: "m", Enabled: true},
		{Provider: "p", Group: "beta", Model: "m", Enabled: true},
		{Provider: "p", Group: "disabled", Model: "m", Enabled: true},
		{Provider: "p", Group: "alpha", Model: "m", Enabled: true},
	}}}
	cfg.RequestRewriteRules = []config.RequestRewriteRule{
		{Name: "exact", Alias: "chat", ProviderGroups: []config.ProviderGroupSelector{{Provider: "p", Group: "missing"}}, Enabled: true},
		{Name: "wild", Alias: "chat", ProviderGroups: []config.ProviderGroupSelector{}, Enabled: true},
	}
	issues, err := ScanConfig(cfg, ScanOptions{CatalogStates: map[string]string{
		CatalogStateKey("p", "alpha"): "not_observed",
		"p":                           "fresh-provider-only",
	}})
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, code := range []Code{CodeAliasTargetGroupMissing, CodeAliasTargetGroupDisabled, CodeAliasTargetProtocolMismatch, CodeRewriteProviderGroupMissing, CodeProviderCatalogStale} {
		if !has[code] {
			t.Errorf("missing group diagnostic %s", code)
		}
	}
	if !has[CodeAliasTargetGroupMissing] {
		t.Fatal("missing group did not fail closed")
	}
	...
	for _, forbidden := range []string{"upstream-secret-alpha", "upstream-secret-beta", "apiKey", "api_key", "apiKeys", "authorization", "x-api-key"} {
		if strings.Contains(serialized, forbidden) {
			t.Fatalf("diagnostic contains sensitive material %q: %s", forbidden, serialized)
		}
	}
}

func TestCatalogStateKeyOnlyFallsBackForDefault(t *testing.T) {
	states := map[string]string{"p": "legacy", CatalogStateKey("p", "alpha"): "alpha"}
	if got := LookupCatalogState(states, "p", "alpha"); got != "alpha" {
		t.Fatalf("alpha state=%q", got)
	}
	if got := LookupCatalogState(states, "p", "beta"); got != "" {
		t.Fatalf("sibling inherited provider state=%q", got)
	}
	if got := LookupCatalogState(states, "p", config.DefaultGroupID); got != "legacy" {
		t.Fatalf("default legacy state=%q", got)
	}
}
...
func TestScanConfigLockedZeroTargetAndAmbiguousProvider(t *testing.T) {
	cfg := config.Default()
	cfg.Providers = []config.Provider{
		{ID: "dup", Groups: []config.ProviderGroup{{ID: config.DefaultGroupID, Protocol: config.ProtocolOpenAIResponses}}},
		{ID: "dup", Groups: []config.ProviderGroup{{ID: config.DefaultGroupID, Protocol: config.ProtocolAnthropicMessages}}},
	}
...
func TestScanConfigAmbiguousIdentitiesDoNotBecomeMissingReferences(t *testing.T) {
	...
	cfg.RequestRewriteRules = []config.RequestRewriteRule{
		{Name: "rule", Alias: "dup-alias", ProviderGroups: []config.ProviderGroupSelector{{Provider: "dup", Group: config.DefaultGroupID}}},
		{Name: "rule", Alias: "dup-alias", ProviderGroups: []config.ProviderGroupSelector{{Provider: "dup", Group: config.DefaultGroupID}}},
	}
