                  ...
                  >
                    <div className="resource-card-top">
                      <div className="resource-card-heading">
                        <div className="resource-card-titlewrap">
                          <div className="inline-meta" style={{ gap: '0.45rem', alignItems: 'center', flexWrap: 'wrap' }}>
                            ...
                            <span className={`badge ${isAuto ? 'outline' : 'live'}`}>
                              {isAuto ? `🏷️ ${t('aliases.autoGenerated')}` : `✏️ ${t('aliases.manual')}`}
                            </span>
                            {isAuto && isLocked ? (
                        ...
                        <p className="resource-card-subtitle">
                          {primaryTarget
                            ? `${primaryTarget.provider}/${primaryTarget.model}`
                            : t('aliases.noTargets')}
                        </p>
                      </div>
                      <div className="resource-card-side">
                    ...
                    <div className="resource-card-meta">
                      ...
                      <div className="resource-meta-item">
                        ...
                        <span className="resource-meta-value">
                          {primaryTarget
                            ? isAuto || aliasTargetIsAutoGenerated(primaryTarget)
                              ? t('aliases.targetAutoAdded', { provider: primaryTarget.provider })
                              : `${primaryTarget.provider}/${primaryTarget.model}`
                            : t('aliases.noTargets')}
                        </span>
                      </div>
            ...
            >
              <div className="panel-header detail-sheet-header">
                ...
                <div className="toolbar">
                  ...
                  {selectedAliasAuto ? (
                    <button
                      type="button"
                      disabled={selectedAliasLocked || !selectedAlias}
                      title={selectedAliasLocked ? t('aliases.autoLocked') : t('aliases.upgradeToManual')}
                      aria-label={selectedAliasLocked ? t('aliases.autoLocked') : t('aliases.upgradeToManual')}
                      onClick={() => {
                        if (!selectedAlias || selectedAliasLocked) {
                          return
                        }
                        const aliasName = selectedAlias.alias
                        setAliasStatus(i18n.t('messages.saving'))
                        void (async () => {
                          try {
                            const updated = await upgradeAutoAlias({ name: aliasName })
                            setAliases((current) => current.map((item) => (item.alias === updated.alias ? updated : item)))
                            setSelectedAliasId(updated.alias)
                            setAliasStatus(i18n.t('messages.saved'))
                            await refreshAll()
                          } catch (error) {
                            setAliasStatus(formatError(error))
                          }
                    ...
                    >
                      {selectedAliasLocked ? t('aliases.autoLocked') : t('aliases.upgradeToManual')}
                    </button>
                  ) : null}
              ...
              <div className="stack-blocks">
                <section className="detail-hero detail-hero-grid">
                  ...
                  <div className="detail-hero-card">
                    <span className="meta-label">{selectedAliasAuto ? t('aliases.autoGenerated') : t('aliases.manual')}</span>
                    <strong>
                      {selectedAliasAuto
                        ? selectedAliasLocked
                          ? t('aliases.autoLocked')
                          : `🏷️ ${t('aliases.autoGenerated')}`
                        : `✏️ ${t('aliases.manual')}`}
                    </strong>
                  </div>
                </section>
                      ...
                      >
                        <div className="target-card-main">
                          ...
                          <div className="target-card-copy">
                            <code>
                                {target.provider}/{target.group || 'default'}/{target.model}
                            </code>
